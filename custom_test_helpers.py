    ...
    for test_method_name in test_method_names:
        ...
        def new_test_method(self, *args, **kwargs):
            ...
            for assertion_method_name in ASSERTION_METHOD_NAMES:
                ...
                orig_methods[assertion_method_name] = orig_method
                def make_wrapper(orig_method, assertion_method_name):
                    @functools.wraps(orig_method)
                    def wrapper(*args, **kwargs):
                        class_counters[assertion_method_name](*args, **kwargs)
                        return orig_method(*args, **kwargs)
                    return wrapper
                wrapper = make_wrapper(orig_method, assertion_method_name)
                mock_method = unittest.mock.Mock(wraps=wrapper)
                setattr(self, assertion_method_name, mock_method)
                new_methods[assertion_method_name] = mock_method

            new_test_method.wrapped_methods = new_methods

            def get_total_call_count(method_names=None):
                if method_names is None:
                    method_names = ASSERTION_METHOD_NAMES
                return sum(
                    new_methods[method_name].call_count
            ...
            new_test_method.get_total_call_count = get_total_call_count
